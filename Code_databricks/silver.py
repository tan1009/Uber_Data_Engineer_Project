...
def rides_bulk():
    df = spark.readStream.option("skipChangeCommits", "true").table("bulk_rides")
    df = df.withColumn("booking_timestamp",col("booking_timestamp").cast("timestamp"))
    df = df.withColumn("pickup_timestamp",col("booking_timestamp").cast("timestamp"))
    df = df.withColumn("dropoff_timestamp",col("booking_timestamp").cast("timestamp"))
    return df
